...
* **A261 Thread.Start 吃实例方法组/闭包：编译通过、调用即崩（2026-09-09 发现，待修）**：`Thread.Start(p.InstanceEntry)` 与 `Thread.Start(() => { p.InstanceEntry(); })` 都能编译通过（Start 返回 true、线程正常创建），线程体调用委托时 SIGSEGV——ThreadStart 委托是裸函数指针（GenForm 注释明言 "delegates are plain function pointers"），实例绑定需要 `this` 语境、捕获闭包需要环境块，两者都没有，函数指针指向的调用约定不匹配直接跳野指针。复现探针（两文件均为最小形态）：实例方法组版 = class P2 { void InstanceEntry(){...} static void Main(){ Thread.Start(new P2().InstanceEntry) } }；闭包版 = P3 同形 `Thread.Start(() => { p.InstanceEntry(); })`（本轮在 _scratch/hts_probe2/3.zan 实测，均 rc=139）。正确姿势是 ImageHttp/ExportJob 的静态 Job 通道（静态入口 + 静态队列/Mutex 传状态，本轮 HttpTableSource 已照此落地）。**待修方向（rule 10 登记）**：checker 对方法组→委托转换判 staticness，实例方法组/含捕获 lambda 喂给无语境委托类型时给出编译错误（"instance method group requires a compatible delegate with receiver binding"）；或 irgen 为委托值补 (fn, ctx) 双字表示——后者动全部委托调用点与 ABI，前者一行拒绝即可止血。修前 stdlib 全库靠约定规避（现有 Thread.Start 调用点全部静态，无一踩雷）。
...
* **zanc 单输入漏收同目录 sibling .zan（2026-09-10 备案，临时绕行在用）**：`zanc path/to/main.zan`（单输入 + --auto-stdlib）不再把同目录兄弟 .zan 收进编译（goldminer src/main.zan+src/Game.zan、wuwei 七文件、_scratch/ns2 最小探针全部 "undefined type 'Rng'/'Game'/…"；同两文件作为多输入显式给出则 19 文件编译过）。dde3b503（9/10 03:38）此前同命令全绿，回归窗口在其后 ~05:32 的 zanc 重烘（并行 wasm 车道在途 irgen_expr.c/main.c 未提交改动高度嫌疑，checker 路径为何受 irgen 改动波及未深究）。**绕行**：多源工程构建命令显式列出全部 .zan（goldminer APK 已按此重出）。**正解待办**：并行车道落地后重验单输入多文件工程；若仍挂，根修 main.c 的项目文件收集（同目录 *.zan 自动纳入），加 conformance 多文件工程用例。影响面：所有多文件模板（wuwei/goldminer/ra2/…）与 IDE 单文件编译心智。
