...
* **A304 [P2/安全] 已修 HttpClient 请求行注入：path 中的 CR/LF/SP/NUL/DEL 撕裂请求行（2026-09-11 HttpClient 检测轮）**：`BuildRequestHead` 把调用方传入的 `path` 原样拼进请求行，`/x HTTP/1.1\r\nHost: evil\r\nX: y` 会把一行撕成多行、向服务器走私第二个请求/伪造头部（探针 `_scratch/httpaudit/guard_request.zan`：修复前 crlfLines=7、evilHost=1）。SetHeader/multipart 有 IsValidHeader/ValidateMultipartToken 同类防线（A289），请求行是最后一个无防线的入口。修：新增 `HttpClient.IsRequestTargetSafe`（拒绝 CR/LF/NUL/DEL/SP，query 的 ?&=% 与 UTF-8 原样放行），`BuildRequestHead` 校验失败抛 HttpRequestException，两个下载通道（DownloadRangeOnceAsync/DownloadBinaryOnceAsync）手写请求行处校验失败走 SetDownloadError+连接清理返回 -1。**回归**：14 个 HTTP conformance（cookie_domain/header_injection/chunk_len_overflow/framing/smuggling/keepalive/redirect/cookies/binary/timeout/upload_bytes/parser_hardening/https_binary_body/bytes_redirect）全部 golden 匹配；修复后探针 plain 放行、crlf/lf/space 全拒绝。
* **A305 [P2/流程] A288/A289 的 stdlib 半曾随并行会话提交丢失，本会话补入库（2026-09-11 HttpClient 检测轮）**：commit 8b24aecb（A272-A301 修复批）的测试半（cookie_domain_match/header_injection_guard/http_chunk_len_overflow 三个 conformance）已入库，但配套 stdlib 半——HttpFramer 分块长度 64 位累加、HttpServer/Worker.SetHttpLimits 的 SetMaxRequestBytes 钳制 [1KiB,1GiB]、CookieJar.DomainAttributeOk（RFC 6265 §5.3 第 6 步）、HttpClient.SetHeader 的 IsValidHeader 与 ValidateMultipartToken、MqttClient CONNACK remainLen>16MiB 断开——只存在于工作树、从未进任何提交（git log -S 全历史无 DomainAttributeOk；`git show 8b24aecb:stdlib/...` 无这些 hunk，而同提交的测试调用它们）。TASKS.md 却记为"已修"。本会话审计发现后随 A304 一并提交入库；14 个 conformance 复跑 golden 全过。**教训**：提交"测试+stdlib 成对"的修复批时，`git status`/diff stat 必须核对 stdlib 侧文件真的进了提交（并行会话共享工作树时选择性提交极易只带上测试半）。
