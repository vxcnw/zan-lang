...
* **A271 编译器/运行时/stdlib 全量代码审计（2026-09-11，六路并行代理 + 逐条最小探针复验）**：范围=词法/语法/检查器、irgen（核心/调用/泛型/ARC）、runtime（内存/字符串头/文件/计时/调度/IO/同步）、stdlib 安全面。**探针全在 `_scratch/audit2/`（先看 README.md）**。**已证伪两条，勿再登记**：① 「`await Task.Delay(long.MaxValue)` 有符号溢出→立即触发」不成立（探针 `delaymax2.zan`：打印 start 后睡死，5s 超时仍未醒；`rt_sched.c:291` 的加法溢出是 UB 但当前无可观察误行为）；② 「`irgen_emit.c` 的 `fields[32]/names[32]` 是缓冲区溢出」不成立（1802-1803、1983-1984 都有 `<32` 守卫，是**静默截断**，names 侧无可见症状；fields 侧仅 A281 那一种形状暴露）。以下 A272-A291 均为探针实测或读码确证项，按严重度排列。**2026-09-11 晚本轮修复：A272-A286、A288、A289 全部，A290 的转义半，A291 的 ③④⑥⑦，另新增 A294（已修）；仍留 A287、A290 的 seed 半、A291 的 ①②⑤⑧、A292，以及新登记的 A293（见各条尾注）。**本轮收尾在 standard 档（785 项）里又归因出 A295/A296/A297/A301（均已修：JSON 数字 token 字面量、arpg Link 判空、HttpsServer 实例调用、Pinyin CRLF）、A298（三个 forwarder 用例 flaky，未修）、A299（selfhost null-safety，已修）、A300（TaskJoin 拉入假阴性，根因=seed 扫描器 chain 双重清空，已修），另登记 A302（tunnel leakcheck 仍可达，未修），并确认 4 项 Gui/GL 红（pagination/transfer/listview 哑停/runtime_gui_gl_3d 目标未构建）来自并行会话在途改动；本树 standard 档当前无法全绿，逐条见尾注。****

* **A272 [P1] 已修 方法体内裸名实例属性赋值让 zanc 段错误（2026-09-11 审计发现，未修）**：`class A { private int b; public int P { get { return b+1; } set { b = value; } } void M() { P = 100; } }`——`P = v;`（隐式 this，属性带自定义 setter）编译即 rc=139（SIGSEGV），`P++;` 同。根因：`irgen_expr.c:3058` 的裸名实例属性写法把 `recv_type` 传成 `g->cur_inst`（非单体化流程里为 NULL），而 `emit_property_setter_call` 在 `:1090` 无保护解引用 `recv_type->sym`；getter 对应路径 `:1017-1021` 明写「`recv_type == NULL` 标记 `base.Prop` 读」并做了判定，setter 漏了同一判定（`route_generic_method` 261 行、`subst_type_param_deep` 均容忍 NULL，唯一裸解引用就是 1090）。**绕法**：写 `this.P = v;`（探针 `prop_set_this.zan` 正确输出 101）。auto 属性（`{get;set;}`）走字段槽不受影响；static 属性裸名写走 `:3032` 另一分支（recv_type 非 NULL）不崩。探针 `prop_set.zan` / `prop_inc.zan` / `prop_set_auto.zan` / `prop_set_static.zan`。修法方向：`emit_property_setter_call` 与 getter 一样接受 `recv_type == NULL`（`:1090` 改 `(!is_static && recv_type) ? recv_type->sym : NULL`），并补 conformance 覆盖「裸名 this 属性写/++」。　**修**：getter/setter 的 NULL 接收者改为发诊断（原 LLVMTypeOf(NULL) 崩溃），裸名 set/++ 走实例形状（incdec 分支先判 getter 是否静态）。探针 prop_set→101、prop_inc→11、this.P→101；新增 conformance bare_name_property（write=100/inc=101/static=7/ro=5/custom=4/after=40），其 determinism/leakcheck/arcguard 孪生全绿。

* **A273 [P1] 已修 `static async` 返回结构体：字段读回 0/垃圾（2026-09-11 审计发现，未修）**：`static async Pt MakeAsync()`（Pt 是 struct）→ 调用方 `await` 后所有字段是 0（代理另见 -855664869 一类垃圾值，随栈内容变），无任何诊断。根因：frame result 槽的编解码是类型导向的，`irgen_async.c` 的 `coerce_to_frame_result` 对结构体落 `:62-64 default → LLVMConstInt(i64,0,0)`（把值丢成 0 再塞槽），`coerce_from_frame_result` 对结构体落 `:126-127 default → return res`（把 i64 原样当结构体用；若结构体含指针则是野指针）。类类型返回走指针路径正常（对照组正确）。探针 `async_struct.zan`（预期 34/7，实得 0/0）。修法方向：要么给聚合类型一条真实的内存表示传输路径（按大小/指针槽），要么在 checker 明确拒绝「async 返回 struct」而不是静默产 0。　**修**：发射 async 方法前检查返回类型的 LLVM 映射，聚合即报 "an async method cannot return an aggregate type: the coroutine result slot is one machine word"（原 coerce_to_frame_result 对聚合走 default 常量 0、awaiter 读回 0/垃圾）。新增 diag 用例 async_aggregate_return。

* **A274 [P2] 已修 裸名静态属性读静默返回 0（2026-09-11 审计发现，未修）**：`class A { private static int b = 3; public static int Custom { get { return b+1; } set { b = value; } } public static int GOnly { get { return 5; } } static void M() { Console.WriteLine(Custom); } }` → 打印 0，而 `A.Custom` → 4；auto 静态属性裸名读正常（`{get;set;}` → 7）。即**裸名静态属性读**没有派发到自定义 getter（返回了未初始化的合成槽），与 A272 同属「裸名成员解析」面但方向相反（写正常、读错）。探针 `static_read.zan`（Auto=7 ✓ / GOnly=0 ✗ / Custom=0 ✗ / A.Custom=4 ✓）。绕法：静态属性读一律限定 `类名.属性`。　**修**：裸名静态属性读改走 property_getter_sym + emit_property_getter_call（与 类名.属性 同径），不再读从未写入的合成后备槽。探针 static_read：Auto=7/GOnly=5/Custom=4/限定名 A.Custom=4。

* **A275 [P2] 已修 ≥2^63 的十进制字面量静默变成负值（2026-09-11 审计发现，未修）**：`long x = 18446744073709551615;` → `x == -1`（探针 `lit64.zan` 输出 -1 / true）。`lexer.c:794-799` 对 `strtoll` 的 ERANGE 走 `(int64_t)strtoull(buf,...)` 保留位模式，但 token 不带无符号标志、checker 也没把它定成 ulong，于是一个「值」静默变成另一个值。同类：hex/oct 的 `buf[64]`、bin/dec 的 `buf[128]` 逐字符拷贝到上限即停（`:657/:682/:707/:770`），超长字面量静默截断成另一个数。修法方向：ERANGE 时要么按 ulong 定型（token 加 unsigned 位）要么直接报「字面量超出 long 范围」；缓冲截断同理应报错。　**修**：ERANGE 的十进制字面量按 ulong 定型（保留位模式），赋给 long 由 checker 报 narrowing。探针 lit64 现 rc=1、消息 "narrowing conversion from 'ulong' to 'long'"；新增 diag 用例 ulong_literal_narrowing。

* **A276 [P2] 已修 类没有任何构造函数时 `new C(args)` 静默丢弃实参（2026-09-11 审计发现，未修）**：`class C { public int A = 1; public int B = 2; }` + `new C(5, 6)` → 编译零诊断，对象只用字段初始化器（探针 `newargs.zan` 输出 1/2，5/6 从未求值）。`irgen_expr.c:7281-7287` 的「没有构造函数接受 N 个实参」诊断以 `type_has_ctor(g, sym)` 为门——类连一个 ctor 都没有时反而**不报**。修法：`ctor_arg_list.count > 0` 且无任何 ctor 可接受实参时一律报错（去掉 `type_has_ctor` 门或改成「存在可接受重载」判定）。　**修**：去掉 type_has_ctor 门，ctor_arg_list.count > 0 即查重载、无可接受者报错。新增 diag 用例 ctor_arg_count；并修掉该诊断暴露的 tests/gui/datatable_sparse_page_test.zan 两处无效调用（new SparseSource(pageSize) 改无参，其后紧跟 Setup(pageSize)，行为不变），该用例恢复通过。

* **A277 [P2] 已修 17+ 参数 extern 按裸名调用：编译器栈越界写 + 伪造的 LLVM 校验错误（2026-09-11 审计发现，未修）**：`irgen_call.c:5029-5034` 声明 `LLVMTypeRef ptypes[16]`，`if (nparams > 16) nparams = 16;` 只钳住后面的 `ptypes[k]` 循环，`LLVMGetParamTypes(fn_type, ptypes)` 无容量参数、按 `LLVMCountParamTypes` 全量写（60 参 = 480B 写进 128B 栈数组）。症状：`error: LLVM verification failed in main: Call parameter type does not match function signature!`（第 16 个实参起仍是 i64，未被 coerce），报错点离真实原因很远；实测 p60 未崩、p17 也以该错误失败，但越界写是实打实的 UB。触发条件是**裸名**路径（`:5019` 的 `LLVMGetNamedFunction` 兜底）；限定名 `A.big17(...)` 过了校验、只在链接期因符号不存在失败（探针 `p17.zan` / `p60.zan` / `p17b.zan`）。修法：按 `nparams` 动态分配 `ptypes`（或 `if (nparams > 16) nparams = 16;` 之后先截断再传一个 16 容量的拷贝——但 LLVM 无此 API，只能动态分配）。　**修**：ptypes 按 nparams 动态分配（原 16 元素栈数组被 LLVMGetParamTypes 全量写入 = 越界写）。探针 p17/p60 现干净失败于链接期 undefined reference to big17/big60，不再有伪造的 LLVM 校验错误；新增 diag 用例 extern_many_params。

* **A278 [P2] 已修 同一行写的 `#endif` 被吞掉，文件剩余内容整体消失（2026-09-11 审计发现，未修）**：`#if 0 x #endif` 这种把条件内容与 `#endif` 写在同一行的形态，`lexer.c:1184-1189` 在非活跃分支里「跳到行尾」，同一行的 `#endif` 永不被处理 → 只报一条 `warning: unterminated #if/#ifdef`，其后所有声明被吞（本探针因此没有 Main，链接报 `undefined reference to WinMain`）。用户看到的是「莫名少了一大半个文件 + 链接错误」，warnings-only 流程下尤其难查。探针 `endif_sameline.zan`。修法：非活跃分支跳过时仍要逐字符识别本行的后续 `#` 指令（或至少在行内发现 `#endif/#else/#elif` 时交回指令处理）。　**修**：非活跃分支跳行时仍识别本行后续 # 指令（#endif/#else/#elif 交回指令处理）。新增 conformance pp_endif_sameline → 7。

* **A279 [P2/安全] 已修 `Encoding.UrlDecode` 丢 `%00` 及其后内容（2026-09-11 审计发现，未修）**：`Encoding.UrlDecode("a%00b")` → `"ab"`（`.Length == 2`，探针 `urldecode.zan`）。`stdlib/System/Text/Encoding.zan:284-297` 用 `CharFromCode(0)` 生成含内嵌 NUL 的 1 字节串再 `Append`，Zan 字符串以 NUL 结尾，拼接按 C 串语义丢掉了 NUL 及其后内容。安全面：把 `%00` 当校验绕过手段（`..%00`、后缀过滤、路径/白名单比较）在「先 UrlDecode 再校验」的代码里会静默失效。修法：解码走 `StringBuilder` 的字节级追加（不经过 NUL 截断的 string 拼接），或对 `%00` 明确报错/替换。　**修**：根治点在编译器而非 stdlib——StringBuilder 的 string 参数追加改走 emit_string_buffer_len（ARC 头里的字节数）而不是 emit_cstr_len_of（strlen），memcpy 后补 emit_string_len_set 标记精确长度，于是 UrlDecode 用 CharFromCode(0) 产出的 1 字节 NUL 串能原样追加（irgen_call.c 的 sb_append 路径）；同类「二进制串/内嵌 NUL」的拼接一并受益。新增 conformance urldecode_nul：len=5、b0..b4=97/0/98/0/99。

* **A280 [P2] 已修 深右递归表达式让编译器静默栈溢出（无任何诊断）（2026-09-11 审计发现，未修）**：`a = a = ... = 1`（10 万层）与 `1?1:1?1:...`（10 万层）→ zanc 以 `STATUS_STACK_OVERFLOW (0xC00000FD)` **无输出**死亡（cmd 取原始退出码，探针 `deep_assign2.zan` / `deep_ternary2.zan`；5 万层正常）。括号嵌套 20k/30k 则在 256 层正常报 `expression nesting too deep (max 256)`——说明 `ZAN_PARSER_MAX_EXPR_DEPTH(256)` 守卫覆盖了 primary/paren 路径，但**赋值右递归与条件表达式右递归**没进这条守卫（`parser.c:2002`、`:1942-1944` 附近）。附带：括号深嵌套时诊断放大严重，30k 层输出 6,048,681 字节（同样一条错误重复上千遍，`deep_paren2.zan`）。修法：把深度计数推进赋值/三元右递归（或改迭代），并把同一位置的重复诊断去重。　**修**：赋值右递归与三元右递归纳入 ZAN_PARSER_MAX_EXPR_DEPTH 守卫，同位置重复诊断去重。7 个深探针（deep_paren/deep_paren2/deep_assign/deep_assign2/deep_ternary2/da_100000/dt_100000）现全部 rc=1、输出约 3.4KB、消息 "expression nesting too deep (max 256)"（此前为无输出的 STATUS_STACK_OVERFLOW 或 6,048,681 字节重复文本）；新增 diag 用例 expr_nesting_depth。

* **A281 [P3] 已修 generic 类的第 33 个 T 型字段起被静默截断 → 误导性 unresolved call（2026-09-11 审计发现，未修）**：`class Box<T> { public T F00..F32; public string Dump() { return F32.ToString(); } }`（33 个 T 型字段）→ `error: unresolved call 'F32.ToString': 'F32' is not a known variable, type, or namespace`。根因：`irgen_emit.c:1782` 的 `fields[32]` 扫描表在 `:1983-1984` 处**有 `<32` 守卫**（不是溢出），第 33 个字段不进表 → `tp_scan_is_tp_field` 判否 → 该方法不被认为使用类型参数 → 只发擦除版 → 成员解析失败。探针 `fields33c.zan`。同类 `names[32]`（局部/参数）也是截断但没有可观察症状（33 参数的泛型方法正常，`names33.zan`）。修法：两张表改动态数组，或超限直接报「类型参数成员扫描表已满」。　**修**：tp_use_scan 的 names/fields 改 realloc 动态数组（配 tp_scan_free）。新增 conformance generic_field_33 → "first mid last"。

* **A282 [P3] 已修 条件编译嵌套 ≥32 时越界写 cond_stack/cond_seen_true（2026-09-11 审计发现，未修）**：`lexer.c` 的 `#if/#ifdef/#ifndef` 在 `cond_depth >= ZAN_PP_MAX_COND_DEPTH(32)` 时走 else 分支，仍写 `cond_stack[cond_depth]` / `cond_seen_true[cond_depth]`（`:442-444`、`:457-459`、`:473-475`）——index 32 越界；`cond_stack[32]` 与紧随其后的 `cond_depth` 相邻，该写把深度清零后 `++` 成 1，嵌套栈从此错位。实测（探针 `cond33.zan` / `cond33b.zan`）该路径已同时发出 `error: conditional-compilation nesting too deep`、编译 rc=1 失败，故**未观察到误编译**；但越界写本身是真 UB，且 `cond_seen_true[32]` 写到相邻成员之外。修法：else 分支只报错、不写数组（后续 `#endif` 用「已报错的哨兵深度」处理）。　**修**：cond_depth 达上限的 else 分支只报错、不再写 cond_stack/cond_seen_true（越界写即 UB）。新增 diag 用例 pp_nesting_depth（33 层 #if）。

* **A283 [P3] 已修 字面量/维度表的静默截断与误导诊断（2026-09-11 审计发现，未修）**：① 字面量缓冲（见 A275 同类）；② `parser.c:408-424` 的 `ranks[16]` 使 17 维数组类型静默丢维，checker 随之报**误导性**错误 `array initializer has 1 element but the dimensions describe 65536`（探针 `dims17.zan`；16 维正常 `dims16.zan`）。该截断顺带使 `irgen_expr.c:7060-7085`「`dims[16]` 钳位但把 raw `rank` 传给 `zan_mdarray_alloc` → 越界读 dims[16..rank-1]」这条**不可达**（17 维在 checker 就被拦下），记为 latent、暂不必修。　**修**：守卫改 `rank <= 16 && nranks < 16`（原 `nranks < 16` 漏掉单括号 rank=17 的情形），并给 new 表达式维度列表加同样守卫。探针 dims17/dims17b 现报 "array rank specifier is too deep (max 16)"；新增 diag 用例 mdarray_rank。

* **A284 [P1/runtime] 已修 同一 handle 上并发读与 Close 的 `FILE*` use-after-free（2026-09-11 审计，读码确证，未复现）**：`rt_file.c` 的 read/write/seek/tell/flush/eof 都是「锁内解析出 `FILE *f`，解锁后使用」（`:632-660` 等，注释明说 blocking IO 不持锁）；`zan_file_close` 也在锁内 `s->open=0; s->fp=NULL`，但把 `fclose(f)` 放到解锁之后（`:662-683`）。handle 的 generation 只挡「之后重新解析」，挡不住已解析出的裸 `FILE*`：同 handle 一协程读、另一协程关 → 读侧 `fread/fseeko` 打在已 free 的 FILE 上。`stdlib/System/IO/FileStream` 自身无锁，`Thread.Start` 里读 + 主线程关就是现成形状。与 A86 族「fd 复用」同源。修法：close 时把 `fclose` 放回锁内（或加每 handle 的「使用中」计数），并把 FileStream 的手柄生命周期写进契约。　**修**：handle 加使用中计数（pin/unpin）串行化「已解析出的 FILE*」与 Close：读/写/seek 期间 close 不 fclose，计数归零才关。审计为读码确证、无复现探针，验收取 smoke 档 runtime_file* 全绿。

* **A285 [P1/runtime] 已修 Windows IOCP 多 worker：blocking 完成项丢失后无第二条投递路径 → 永久挂死且进程不退出（2026-09-11 审计，读码确证；**已登记「唤醒包偶发丢失」的根因定位**）**：① 投递端 `rt_io.c:2590-2600` 的 `dns_wake_notify()` 调 `PostQueuedCompletionStatus(..., (ULONG_PTR)-2, NULL)` **忽略返回值**，而 `-2` 包是 blocking/DNS 完成项的唯一唤醒手段；② 兜底端 `co_wait_io` 在 `GetQueuedCompletionStatusEx` 超时/失败时只 `dns_timeout_scan()` 就 `return`（`:3700-3704`），**从不 `dns_drain()`**（对比单线程 Windows 路径 `:2112-2114` 是 `dns_timeout_scan(); return dns_drain();`，多 worker 版漏了后者）。一旦该包丢失/投递失败，已完成作业永远躺在 `g_blocking_done`，等待协程不 resume，`g_co_stop` 也因 `g_blocking_inflight != 0` 永不置位 → worker 空转不退出。注册项里「偶发丢失」对应①，「丢失后无法自愈」对应②。修法：`!ok` 分支补幂等 `dns_drain()`；`dns_wake_notify` 失败时置「有完成项」标志由下一轮循环兜底。　**修**：dns_wake_notify 检查 PostQueuedCompletionStatus 返回值并置「有完成项」标志；co_wait_io 的 !ok 分支补幂等 dns_drain()（与单线程 Windows 路径对齐，原来多 worker 版漏了这一句）。读码确证项，smoke 档 runtime_* / 调度相关用例全绿。

* **A286 [P2/安全] 已修 SSRF 判定漏 IPv6 过渡/兼容地址（2026-09-11 审计，读码确证）**：`rt_io.c:580-605` 的 IPv6 分支只拦 ULA `fc00::/7`、link-local `fe80::/10`、多播 `ff00::/8`、文档 `2001:db8::/32`、v4-mapped `::ffff:a.b.c.d`，其余一律 `return 1`（安全）。于是 `2002::/16`(6to4，如 `2002:0a00:0001::` 内嵌 10.0.0.1)、`64:ff9b::/96`(NAT64)、`2001::/32`(Teredo)、IPv4-compatible `::a.b.c.d` 全部被判「安全」。该函数是 `TcpClient.ConnectAsync` 对每个 DNS 候选地址的准入判据（任一不安全即整体拒绝），在启用 6to4/NAT64/Teredo 的主机上可绕过内网过滤。修法：补齐这些前缀（解出内嵌 IPv4 后递归判定），并考虑「不在白名单内的过渡前缀一律拒绝」。　**修**：IPv6 分支补齐 Teredo（2001::/32，字节 4..7 / 12..15 取反）、6to4（2002::/16，偏移 2）、NAT64（64:ff9b::/96，偏移 12）、IPv4-compatible（偏移 12），解出内嵌 IPv4 后递归判定。tests/runtime/rt_io_addr_test.c 加 8 条过渡形态向量，runtime_io_addr PASS。
...
* **A288 [P2/安全] 已修 若干部件的长度解析无上限/可回绕（2026-09-11 审计，读码确证）**：① `HttpFramer.zan:270-287` 分块长度用 32 位 `int` 逐位 `size = size*16 + d`，每位后只判 `size > maxBodyBytes`：发 `"100000000\r\n"` 时最后一位使 `size` 由 `0x10000000` 回绕为 0 → 被当终止块，剩余字节作为下一条流水线请求解析（请求框架错位/走私面）。默认 `SetMaxRequestBytes` 1 MiB 会先 413，需应用把上限调到 ≥256 MiB（268435456）才可利用——但那个值正是「大文件上传」场景的常见配置，而 `HttpServer.zan:84` 对该参数无上限钳制。客户端 `HttpClient.ParseHex` 有 16 MiB 硬上限，其注释却假定「服务器端早有同样上限纪律」，与事实不符。② `MqttClient.zan:231-232` 的 CONNACK 快路径 `ReadRemainingLenAsync` 只限 varint 4 字节，`remainLen` 可达 268435455（256 MiB），随即 `ReadBytesAsync` 分配 `need+1` 两个缓冲（`:204-206`）→ 单次连接尝试约 512 MiB 分配，未认证对端即可拖垮客户端；`MqttBroker.zan:576` 侧有 16 MiB 拦截，两侧不对称。修法：分块长度改用 64 位并在每位后判上限；`SetMaxRequestBytes` 加上限钳制；CONNACK 路径套用 16 MiB 上限。　**修**：①HttpFramer 分块长度改 64 位、每位后判上限（回绕即 413，不再把 100000000 当终止块）；②SetMaxRequestBytes 在 HttpServer 与 Worker.SetHttpLimits 钳到 [1 KiB, 1 GiB]；③MqttClient CONNACK 路径 remainLen>16 MiB 即断开。新增 conformance http_chunk_len_overflow（3 组 ok、failures=0；用 SetMaxRequestBytes(1 GiB) 使 8 位前缀合法、只让第 9 位真实溢出）。

* **A289 [P2/安全] 已修 Cookie 域校验缺失与请求头注入（2026-09-11 审计，读码确证）**：① `System/Net/Http/Client/CookieJar.zan:135-139` 解析 `Set-Cookie` 时只要出现 `Domain=` 就无条件写入 `c.domain` 并置 `hostOnly=false`，从不校验该域是否与**发起响应的请求主机**域匹配（RFC 6265 §5.3 第 6 步缺失；前导 `.` 只做去除）→ 攻击者可控主机/子域可为主域或他域种 cookie（会话固定、cookie 污染、超级 cookie），共享 jar 的客户端尤其危险。② `HttpClient.zan:298-301` 的 `SetHeader` 把 `name + ": " + headerValue` 直接入表、不校验 CRLF；多部件构造 `:2469-2471`、`:2510-2511` 把 `name`/`fileName` 原样拼进 `Content-Disposition`——同库 `HttpResponse.SetHeader`、`WebDavClient.ValidateHeader`（`:283-288`）都显式拒绝控制字符，唯客户端这两处漏了。修法：补域匹配（含 public-suffix 级别的宽松处理也要拒绝跨注册域）；客户端 SetHeader/multipart 复用 `ValidateHeader`。　**修**：①CookieJar 补 RFC 6265 §5.3 第 6 步域属性匹配（拒绝 IP 主机、无点域、不匹配域、后缀混淆 evil-example.com 对 example.com）；②HttpResponse.SetHeader 与 HttpClient.SetHeader 复用 CRLF/控制字符校验，multipart 的 name/fileName 走 ValidateMultipartToken。新增 conformance cookie_domain_match（parentScope=1/crossDomain=0/crossReplay=/suffixConfusion=0/tld=0/ip=0/hostOnly=host=1/near=0）与 header_injection_guard（respInject=0/respColon=0/cliEvil=0/cliLines=0/tokenQuote=rejected/tokenCrlf=rejected/multipartField=rejected）。
...
* **A293 [P1] async 方法内「嵌套的非 async 函数」抛出的异常被 awaiter 捕获后，awaiter 的局部变量全为空（2026-09-11 本轮验证中发现，未修）**：`_scratch/fixv/exc_local*.zan` 四组对照——在 await 的 async 方法体内直接 throw 由 awaiter catch：局部 len=20（正常）；真正挂起后再 throw：len=20（正常）；**但在嵌套的非 async 辅助函数里 throw、由 awaiter catch：所有局部为 NULL/0（len=0）**。辅助函数返回类或 int 都复现，用字面量局部变量也可复现（与 A279 的 StringBuilder 无关）。怀疑点在 `irgen_async.c` 的 EH trampoline（约 1442/1513：landing pad 只恢复了 resume 点，未恢复 awaiter frame 的 locals 槽）。本轮不修：属 async×异常交互的独立域，需先做 IR 层最小定位；探针与判定留在 `_scratch/fixv/`。

* **A294 [P2/测试基建] Gui conformance 用例没有 TIMEOUT，桌面竞争下一次哑停让整档 ctest 永久挂死（2026-09-11 本轮实测，已修）**：`conformance_gui_listview_scrollbar_drag` 在两次 ctest 运行里各挂住 14 分钟以上（进程 CPU 仅 0.1-0.3s、窗口 listview-drag 存在但不返回），`conformance_gui_childwindow_shape` 也在本轮以 150s 超时被截断。这些 gui 段的 add_test() 都没设 TIMEOUT，而 glob 出来的 conformance 用例在 CMakeLists 里统一设了 TIMEOUT 120（注释里就写着 without it ctest waits out its 1500s default and one hung case alone costs 25 minutes）——gui 段漏了。**修**：CMakeLists.txt 末尾标签循环里给全部 conformance_gui_* 设 TIMEOUT 240，哑停变成一条失败而不是整档停摆。
* **A295 [P1/stdlib 正确性] JSON 解析丢浮点/超长整数 token 的原字面量：AsString/ToJson 不再无损，conformance_jwt_hs256 在 HEAD 就红（2026-09-11 本轮 ctest 归因中发现，已修）**：`conformance_jwt_hs256` 的失败只有一行差异——期望 `exponent-nbf=0:invalid nbf`、实际 `1:`（被当成合法 nbf 接受）。干净 worktree（34a3589e）的 zanc 编译同一用例输出同样差这一行，故与本轮未提交改动无关。机制：`Jwt.readNumericDate`（`stdlib/System/Security/Cryptography/Jwt.zan:160-174`）要求数字 token 逐字符都是十进制数字，而 payload 是 `JsonValue.Parse` 出来的；perf 提交 c7791e11（JsonTape 平铺解析器）把 `JsonValue.ParseNumberToken`（`stdlib/System/Json/JsonValue.zan:570-591`）改成「纯整数存 numI、含 . / e 存 numD」且**不再填 numRaw**，`NumText`（`:303-307`）于是回落到 `Convert.ToString(numD)`：`1e2` 格式化成 `100`（全数字 → 误判合法），`200.9` 仍是 `200.9`（所以只有 exponent 这条暴露）。更广的损失是 >18 位整数也走 double 分支，`AsString`/`ToJson` 会输出 `1.23E19` 这种丢精度的近似值。**修**：double 分支本来就是为 `DoubleOf` 切了一次 token 子串，改成复用同一个 raw 填 numRaw（`JsonValue.zan:589-596`）——热路径（≤18 位纯整数）一行未动，分配次数不变。实测：jwt 用例 24 行与 golden 逐字节一致（`exponent-nbf=0:invalid nbf`、`negative-exp`、`overflow-exp` 全对）；`ctest -R "json|jwt"` 47 项 46 过，唯一失败 `selfhost_json_ignore` 为 full 档既有（A299）。`json_tape_roundtrip` 断言的是 `JsonDoc.DoubleAt`（另一条 tape API）不是文本，不在影响面。
* **A296 [P1/编译期诊断×stdlib] conformance_arpg_ui_runtime 在 HEAD 编不过：5 处 'Link' can return null（2026-09-11 本轮 standard 档归因，已修）**：诊断出自 51d2f1e13「Reject null-returning receivers」，859728312 已按同一诊断扫过一轮测试与 stdlib 但漏了这几处：测试 `tests/conformance/arpg_ui_runtime.zan:162-163` 的 `parsed.RunAt(3).Link().At(0)`（2 处），与 `stdlib/Game/Arpg/UiRuntime.zan:1917/1919/1921` 的 `run.Link().Raw()`（3 处，ActivateLink 内）。`ArpgRichTextRun.Link()` 的契约就是「非链接 run 返回 null」（`stdlib/Game/Arpg/RichText.zan:206-214`），诊断是对的。**修**：ActivateLink 改成取一次 `ArpgRichTextLink link = run.Link(); if (link == null) { return false; }` 后复用（前面的 `!run.IsLink()` 守卫不变；真实链接路径行为不变，万一为 null 由「fault 崩溃」变成「返回 false」）；测试同法取局部并判空，null 时打印 `link=null`（回归可见而不是段错误）。实测：编译通过，输出 20 行与 golden 逐字节一致。
* **A297 [P1/stdlib 真 bug] HttpsServer 把实例方法当类型名调用，conformance_https_binary_body 在 HEAD 编不过（2026-09-11 本轮 standard 档归因，已修）**：`stdlib/System/Net/Https/HttpsServer.zan:135` 写的是 `await HttpsServer.HandleConnectionInner(clientSock)`，而 `HandleConnectionInner` 是实例方法（同文件 `:142` 定义），必须走实例——同类兄弟 MqttBroker（`:546`）、WssServer（`:128`）、WebApp（`:369`）都写的 `this.`，只有这一处漏了。诊断「cannot call instance method ... via the type name」自 e59be75a0（09-05）起就在。**修**：改 `this.HandleConnectionInner(clientSock)`。实测：编译通过，输出 14 行与 golden 逐字节一致。
...
* **A299 [P2/selfhost] selfhost_json_ignore 在 HEAD 编不过：selfhost 源码没跟上 null-safety 诊断（2026-09-11 本轮归因，未修，登记）**：zanc 编译 selfhost 编译器源码时报两处——`src/selfhost/irgen.zan:597` `Node fty = sf.type;`、`src/selfhost/irgen_expr.zan:2693` `clone.sval = sname;`（'CloneSubst' can return null）。诊断出自 51d2f1e13（在 HEAD 内），`src/selfhost/` 工作树干净且本轮未触碰，属 full 档既有红；修法是给这两处（及其同类）加判空或改 `?.`，与 A296 同一类机械扫描工作。
* **A300 [P1/编译器×stdlib 注册] conformance_web_typed_binding 在 HEAD 编不过：`Task.WhenAll` 脱糖成 `TaskJoin.WhenAll` 后 TaskJoin 不可解析（2026-09-11 本轮 standard 档归因，未修，登记）**：报错 `unresolved call 'TaskJoin.WhenAll': 'TaskJoin' is not a known variable, type, or namespace (is the class imported and registered in the stdlib map?)`，8 处，全在 `await Task.WhenAll(await Program.Call(Task.Spawn(...)))` 这种「WhenAll 的实参又是一个 await」的形状里；干净 worktree（34a3589e）编译同一用例报同样的 8 处，故与本轮无关。已定位的事实：`stdlib/System/TaskJoin.zan` 确实存在且有 `static async int WhenAll(List<long>)`（`:29`），parser 的 `desugar_task_join`（`parser.c:1415-1431`）把 `Task.WhenAll` 改写成 `TaskJoin.WhenAll`，`main.c` 里有两处镜像这个改写以便按需拉入（token 扫描 `:1030-1037`、另一处种子扫描 `:1243-1250`），但本用例仍然没把 TaskJoin 拉进来（按需拉入的假阴性是这条线的老毛病，skill 里有整节）。**未修理由**：修它要动 main.c 的拉入种子/`TaskJoin` 的 stdlib map 注册，属并行会话正在碰的 async/拉入区，且验证要再跑整档；下一步按「拉入假阴性」四步查（先确认 TaskJoin.zan 的 namespace/文件映射，再看两处镜像是否覆盖 await 嵌套形状）。
* **A301 [P2/Windows 兼容] conformance_tryget_pinyin 在 HEAD 红：pinyin.txt 检出成 CRLF，表构建不剥 \r（2026-09-11 本轮 standard 档归因，已修）**：失败输出是对全部 5 个 pinyin 断言 `FAIL`（`Dictionary.TryGetValue` 的小字典断言全过，只有查表失败）。根因：`stdlib/System/Text/data/pinyin.txt` 的 blob 是 LF、工作区被 `core.autocrlf=true` 检出成 CRLF（`git ls-files --eol` = `i/lf w/crlf`，实测 6763 个 CRLF），而 `Pinyin.Build()` 用 `Split("\n")` 后不处理行尾（`stdlib/System/Text/Pinyin.zan:91-103`）→ 表里每个拼音值都带尾 `\r`（`"yi\r"`），查 `"yi"` 永远不中；Linux CI（LF 工作区）不暴露，Windows 检出必红。这也解释了为什么它是「HEAD 既有」而不需任何其它改动。**修**：构表前剥掉行尾 CR（4 行，同样的坑对用户经 `ZAN_PINYIN_DATA` 传 CRLF 数据文件也适用）。实测：`build/zanc` 编译后输出与 golden 逐字节一致（`tryget-pinyin-ok: 1`）。**同名坑**：同一次归因中还踩到「注释里写反斜杠-r 被工具传输折成真 CR」——两处注释里落了裸 CR 字节，已用 `chr(92)` 拼回（见 skill 的静默陷阱节）。
