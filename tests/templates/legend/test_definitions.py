...
class Definitions(unittest.TestCase):
    def test_all_item_attributes_match_reference(self):
        raw = {r['id']:r for r in table('reference/item')}
        runtime = table('items')
    ...
    def test_all_valid_monsters_and_bosses(self):
        for dest, source in [('monsters','monster'),('bosses','boss')]:
            raw = {r['id']:r for r in table('reference/'+source) if r.get('zhp')}
            runtime = table(dest)
            self.assertEqual({r['id'] for r in runtime}, set(raw))
            for row in runtime:
                for key, field in module.ATTRIBUTES.items():
    ...
    def test_import_is_repeatable_without_clobbering_rewards(self):
        with tempfile.TemporaryDirectory(dir=ROOT/'_scratch',prefix='legend-data-') as tmp:
            path=Path(tmp)/'data';shutil.copytree(DATA,path)
            before={n:(path/f'{n}.csv').read_bytes() for n in ['items','monsters','bosses']}
            module.enrich(path)
            self.assertEqual(before,{n:(path/f'{n}.csv').read_bytes() for n in before})
