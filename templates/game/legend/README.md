...
联机 PK、行会、沙巴克、交易、排行榜等没有服务器支持，不制造玩家或战报。原始 41 条技能、475 条配方、251 条称谓、52 条套装等保留在 `data/reference/`，不宣称已全部执行；当前运行时技能已接入三职业各八槽配置，原始技能库仍未逐条还原。天书传承已接入（更高系数转移、材料消耗、失败原子性与存档）；宝石成长已接入六槽独立等级、属性聚合、普通升级与 1→5 合成阶段；普通升级和合成都按表驱动且资源不足时不扣除；内功已接入八槽模板规则。藏宝图已接入本地六格抽取循环，但服务器奖池刷新与会员/师徒来源未实现。勋章、龙珠等额外槽位未实现；当前支持十二个穿戴位置。重量、基准售价、堆叠上限等已导入（出售已使用基准售价），但尚未实现负重系统、通用消耗品背包和原版商店经济；不能把这些元数据当成已完成玩法。
